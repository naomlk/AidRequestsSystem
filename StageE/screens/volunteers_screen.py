...
class VolunteersScreen(ctk.CTkFrame):
    def __init__(self, parent, db_connection):
        ...
        title = ctk.CTkLabel(self.header_frame, text="👷 Volunteers Tracker", font=ctk.CTkFont(size=20, weight="bold"),
                             text_color="#0F4C81")
        ...
        # --- SEARCH BAR: search volunteer by ID, first name, or last name ---
        ...
        # CTkComboBox is editable, so this binding refreshes the suggestions while typing.
        try:
    ...
    def load_volunteers_from_db(self):
        ...
        try:
            ...
            self.all_volunteer_rows = rows

            self.display_volunteer_rows(rows)
    ...
    # ========================================================
    # VOLUNTEER SEARCH BAR HELPERS
    # ========================================================
    def display_volunteer_rows(self, rows):
        """Render the volunteer rows inside the table without changing the database."""
    ...
    def filter_volunteer_rows(self, search_text):
        """Filter volunteers by volunteer_id, first name, last name, or full name."""
    ...
    def refresh_search_suggestions(self, search_text):
        ...
        self.volunteer_option_to_id = {
            self.build_volunteer_search_option(row): str(row[0])
            for row in matching_rows[:30]
        }
    ...
    def select_first_search_result(self, event=None):
        if not self.volunteer_search_options:
            return
    ...
    def on_update_button_click(self):
        ...
        if not selected_item:
            messagebox.showwarning("Selection Missing",
                                   "Please click and select a volunteer from the list grid before initiating the update routine.")
            return
    ...
    def open_volunteer_skills_summary(self, volunteer_id, volunteer_name):
            ...
            if skills_list:
                for skill in skills_list:
                    ...
                    for w in [skill_frame, lbl_title, lbl_desc]:
                        w.bind("<Button-1>",
                               lambda event, sid=s_id, sn=s_name, sd=s_desc, sc=s_cert: select_skill_row(sid, sn, sd,
                                                                                                         sc))
            else:
        ...
        def add_new_skill():
            ...
            try:
                ...
                cursor.execute("UPDATE public.b_skill SET requires_certificate = %s WHERE skill_id = %s;",
                               (chosen_cert, target_sid))
                # 2. Insert link into junction relation table
                cursor.execute("INSERT INTO public.b_volunteer_skill (volunteer_id, skill_id) VALUES (%s, %s);",
                               (v_id_clean, target_sid))
                ...
                messagebox.showerror("Database Error",
                                     f"This volunteer already possesses this skill asset entry row assignment or database failed:\n{e}")

        # --- CRUD: UPDATE MUTATION DIALOG MODAL (MINI POPUP WINDOW) ---
        def open_update_mini_window():
            if not selected_row_data["id"]:
                messagebox.showwarning("Target Selection Missing",
                                       "Please click on a skill row layout element above first.")
                return
            ...
            ctk.CTkLabel(edit_win, text="Skill Description Context Overwrite:",
                         font=ctk.CTkFont(size=11, weight="bold")).pack(anchor="w", padx=25, pady=(5, 2))
            ...
            ctk.CTkLabel(edit_win, text="Requires Validated Certificate (Y/N Status):",
                         font=ctk.CTkFont(size=11, weight="bold")).pack(anchor="w", padx=25, pady=(5, 2))
            ...
            def execute_update():
                try:
                    ...
                    lbl_selected_status.configure(
                        text=f"🎯 Selected: {selected_row_data['name']} (ID: #{selected_row_data['id']})",
                        text_color="#1A62E8")
            ...
            ctk.CTkButton(edit_win, text="💾 Save Operational Configurations", font=ctk.CTkFont(size=12, weight="bold"),
                          fg_color="#198754", hover_color="#146C43", height=35, command=execute_update).pack(fill="x",
                                                                                                             padx=25)
        ...
        def delete_selected_skill():
            ...
            if messagebox.askyesno("Confirm Deletion",
                                   f"Are you sure you want to remove '{selected_row_data['name']}' from {volunteer_name}'s profile?"):
                try:
                    cursor = self.conn.cursor()
                    cursor.execute("DELETE FROM public.b_volunteer_skill WHERE volunteer_id = %s AND skill_id = %s;",
                                   (v_id_clean, selected_row_data["id"]))
                    self.conn.commit()
                    cursor.close()

                    lbl_selected_status.configure(
                        text="💡 Click on a skill above to initialize update or delete actions.", text_color="#6C757D")
        ...
        # ========================================================
        footer_crud_panel = ctk.CTkFrame(profile_win, fg_color="#FFFFFF", corner_radius=10, border_width=1,
                                         border_color="#E9ECEF")
        footer_crud_panel.pack(fill="x", padx=20, pady=(0, 15), ipady=10)

        # ---- SECTION 1 : CREATE (ADD SKILL INTERACTION FRAMEWORK DECK) ----
        add_title = ctk.CTkLabel(footer_crud_panel, text="➕ Assign New Registered Skill to Profile:",
        ...
        btn_action_add = ctk.CTkButton(footer_crud_panel, text="Assign Skill", font=ctk.CTkFont(size=12, weight="bold"),
                                       fg_color="#1A62E8", hover_color="#1452C7", height=30, width=100,
                                       command=add_new_skill)
        ...
        lbl_selected_status = ctk.CTkLabel(footer_crud_panel,
                                           text="💡 Click on a skill above to initialize update or delete actions.",
        ...
        ctk.CTkButton(btn_group_box, text="Update Selected", font=ctk.CTkFont(size=11, weight="bold"),
                      fg_color="#198754", hover_color="#146C43", width=110, height=28,
                      command=open_update_mini_window).pack(side="left", padx=2)
        ctk.CTkButton(btn_group_box, text="Delete Selected", font=ctk.CTkFont(size=11, weight="bold"),
                      fg_color="#DC3545", hover_color="#BD2130", width=110, height=28,
                      command=delete_selected_skill).pack(side="left", padx=2)
    ...
    def delete_selected_volunteer(self):
        ...
        if not selected_item:
            messagebox.showwarning("Selection Missing",
                                   "Please click on a volunteer from the list before requesting a deletion.")
            return
        ...
        if confirm:
            try:
                ...
                self.conn.rollback()
                messagebox.showerror("SQL Transaction Failed",
                                     f"Database engine refused delete constraint execution:\n{e}")
    ...
    def open_volunteer_form(self, edit_mode=False, data=None):
        ...
        def save_form_data():
            try:
                ...
                if not v_id or not f_name or not l_name or not phone:
                    messagebox.showwarning("Validation Error", "ID, Names, and Phone fields must be fully populated.",
                                           parent=form_window)
                    return
                ...
                try:
                    ...
                    messagebox.showerror("Format Error",
                                         "Latitude and Longitude must be valid decimal numbers (e.g., 31.7683).",
                                         parent=form_window)
                    return
                ...
                if edit_mode:
                    ...
                    """
                    cursor.execute(sql, (f_name, l_name, phone, has_eq, cnt_val, lat_val, lon_val, date_val, email_val,
                                         act_val, v_id))
                else:
                    sql = """
                    ...
                    """
                    cursor.execute(sql, (int(v_id), f_name, l_name, phone, has_eq, cnt_val, lat_val, lon_val, date_val,
                                         email_val, act_val))
        ...
        btn_save = ctk.CTkButton(form_window, text="💾 Save Profile Changes", font=ctk.CTkFont(size=13, weight="bold"),
                                 fg_color="#198754", hover_color="#146C43", height=38, corner_radius=6,
